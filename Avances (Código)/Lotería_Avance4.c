...
// Nombres de patrones disponibles para seleccion previa
static const char *g_nom_patron[] = {
    "Libre", "Diagonal Principal", "Diagonal Invertida",
...
/* ================================================================
 *  MODULO 6 - PROCESO: BARAJAR, MARCAR Y LÓGICA
 * ================================================================ */
...
{
    switch (p) {
        case PAT_DIAG_P:  return t[0][0] && t[1][1] && t[2][2] && t[3][3];
        case PAT_DIAG_S:  return t[0][3] && t[1][2] && t[2][1] && t[3][0];
        case PAT_FILA:
            for (int i = 0; i < TAM; i++)
        ...
        case PAT_COLUMNA:
            ...
            return 0;
        case PAT_ESQUINAS: return t[0][0] && t[0][3] && t[3][0] && t[3][3];
        case PAT_CENTRO:   return t[1][1] && t[1][2] && t[2][1] && t[2][2];
        default: return 0;
    }
}
...
void salida_reglas(void) // Texto estático de las reglas del juego.
{
...
void flujo_jugar(void) // Empieza el juego
{
    proceso_barajar();
    ...
    while (g_partida.carta_actual < TOTAL_CARTAS) {
        ...
        if (op == 2) { util_limpiar_pantalla(); salida_todos_tableros(); util_pausar(); continue; }
        if (op == 3) { salida_historial_cartas(); util_pausar(); continue; }
        if (op == 4) { printf("  Saliendo de la partida...\n"); util_pausar(); return; }
        ...
        if (!hay_ganador) printf("  Ningun ganador todavia. Siguen jugando...\n");
        util_pausar();
 ...
 *
 *  Funciones probadas:
 *    - proceso_aplicar_habilidad : activa bloqueo con probabilidad
 *                                  PROB_HABILIDAD% en cartas especiales
 *    - salida_patron_objetivo    : dibuja en ASCII el patron a completar
 *    - salida_reglas             : muestra reglas incluyendo habilidades
 *    - proceso_revisar_patron    : evalua si un tablero cumple el patron
 *    - proceso_patron_chico      : delega segun configuracion de patron
 * ================================================================ */
int main(void)
{
    ...
    printf("  ========================= PRUEBA 1 =========================\n");
    printf("  Verifica que se muestren las reglas con info de habilidades.\n");
    printf("  ============================================================\n");
    ...
    printf("  ======================= PRUEBA 2 ===========================\n");
    printf("  Muestra la mascara ASCII de cada patron disponible.\n");
    printf("  ============================================================\n");

    g_partida.modo_juego = MODO_CHICO;
    const char *nombres_pat[] = {
        "Diagonal principal", "Diagonal secundaria",
        "Fila completa", "Columna completa",
        "4 Esquinas", "Centro 4 casillas"
    };
    for (int p = PAT_DIAG_P; p <= PAT_MAX; p++) {
        printf("\n  Patron %d - %s:\n", p, nombres_pat[p - 1]);
        g_partida.patron_chico = p;
        salida_patron_objetivo();
    }

    printf("\n  ====================== PRUEBA 3 ==========================\n");
    printf("  Saca 20 cartas y reporta cuantas activaron el bloqueo.\n");
    printf("  ============================================================\n");
    ...
    int habilidades_activadas = 0;
    for (int t = 0; t < 20; t++) {
        int idx = g_partida.orden_baraja[t];
        int bloqueo_antes = g_jugadores[1].bloqueado;
        proceso_aplicar_habilidad(idx);
        if (g_jugadores[1].bloqueado > bloqueo_antes) habilidades_activadas++;
    }
    printf("  Habilidades activadas en 20 cartas: %d\n", habilidades_activadas);
    printf("  (probabilidad esperada ~%d%%, resultado puede variar)\n\n",
           PROB_HABILIDAD);

    printf("\n  Lanzando juego completo con habilidades activas...\n");
    util_pausar();
